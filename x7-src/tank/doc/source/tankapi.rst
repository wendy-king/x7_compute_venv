...
Tank has a RESTful API that exposes both metadata about registered virtual
machine images and the image data itself.

A host that runs the ``bin/tank-api`` service is said to be a *Tank API
Server*.

Assume there is a Tank API server running at the URL
``http://tank.example.com``.
...
We issue a ``GET`` request to ``http://tank.example.com/images/`` to retrieve
this list of available *public* images. The data is returned as a JSON-encoded
mapping in the following format::

  {'images': [
    {'uri': 'http://tank.example.com/images/71c675ab-d94f-49cd-a114-e12490b328d9',
...
We issue a ``GET`` request to ``http://tank.example.com/images/detail`` to
...
first public image returned, we can issue a ``HEAD`` request to the Tank
server for the image's URI.

We issue a ``HEAD`` request to
``http://tank.example.com/images/71c675ab-d94f-49cd-a114-e12490b328d9`` to
...
``HEAD`` request::
  ...
  x-image-meta-uri              http://tank.example.com/images/71c675ab-d94f-49cd-a114-e12490b328d9
...
.. note::
  ...
  image data is immutable once stored in Tank
...
We can do the above two activities in a single call to the Tank API.
Assuming, like in the examples above, that a Tank API server is running
at ``tank.example.com``, we issue a ``POST`` request to add an image to
Tank::

  POST http://tank.example.com/images/

The metadata about the image is sent to Tank in HTTP headers. The body
of the HTTP request to the Tank API will be the MIME-encoded disk
image data.
...
Tank will view as image metadata any HTTP header that it receives in a
``POST`` request where the header key is prefixed with the strings
``x-image-meta-`` and ``x-image-meta-property-``.

The list of metadata headers that Tank accepts are listed below.
...
* ``x-image-meta-id``
  ...
  **409 Conflict** will be returned by Tank. The value of the header
  must be a uuid in hexadecimal string notation
  (i.e. 71c675ab-d94f-49cd-a114-e12490b328d9).

  When this header is *not* present, Tank will generate an identifier
...
* ``x-image-meta-store``
  ...
  backing store indicated by the value of the header. If the Tank node
  does not support the backing store, Tank will return a **400 Bad Request**.
...
* ``x-image-meta-size``
  ...
  body *does not match* the value of this header, Tank will return a
  **400 Bad Request**.
...
* ``x-image-meta-checksum``
  ...
  When present, Tank will verify the checksum generated from the backend
...
* ``x-image-meta-is-public``
  ...
  When Tank finds the string "true" (case-insensitive), the image is marked as
  a public image, meaning that any user may view its metadata and may read
  the disk image from Tank.
...
* ``x-image-meta-owner``
  ...
  Tank normally sets the owner of an image to be the tenant or user
...
* ``x-image-meta-property-*``
  ...
  When Tank receives any HTTP header whose key begins with the string prefix
  ``x-image-meta-property-``, Tank adds the key and value to a set of custom,
...
the Tank server knows about.  We take the `uri` field of the image data,
...
first public image returned, we can issue a ``GET`` request to the Tank
server for
``http://tank.example.com/images/71c675ab-d94f-49cd-a114-e12490b328d9/members``
...
a ``GET`` request to ``http://tank.example.com/shared-images/tenant1``.  We
...
request to
``http://tank.example.com/images/71c675ab-d94f-49cd-a114-e12490b328d9/members/tenant1``
...
``DELETE`` request to ``http://tank.example.com/images/1/members/tenant1``.
