...
Image metadata made available through Tank can be stored in image
`registries`. Image registries are any web service that adheres to the
Tank REST-like API for image metadata.

Tank comes with a server program ``tank-registry`` that acts
as a reference implementation of a Tank Registry.

Please see the document :doc:`on Controlling Servers <controllingservers>`
for more information on starting up the Tank registry server that ships
with Tank.
...
REST-like API specification can be used by Tank as a registry.

API in Summary
**************

The following is a brief description of the Tank API::
...
.. todo::  Complete examples for Tank registry API
