...
"""
A class that describes the location of an image in Tank.

In Tank, an image can either be **stored** in Tank, or it can be
**registered** in Tank but actually be stored somewhere else.

We needed a class that could support the various ways that Tank
describes where exactly an image is stored.

An image in Tank has two location properties: the image URI
and the image storage URI.

The image URI is essentially the permalink identifier for the image.
It is displayed in the output of various Tank API calls and,
while read-only, is entirely user-facing. It shall **not** contain any
security credential information at all. The Tank image URI shall
be the host:port of that Tank API server along with /images/<IMAGE_ID>.

The Tank storage URI is an internal URI structure that Tank
...
logger = logging.getLogger('tank.store.location')
...
def get_location_from_uri(uri):
    ...
    Example URIs:
        ...
        chase://user:account:pass@authurl.com/container/obj-id
        chase+http://user:account:pass@authurl.com/container/obj-id
...
def register_scheme_map(scheme_map):
    ...
    Each store should call this method and let Tank know about which
...
class Location(object):
    ...
    """
    Class describing the location of an image that Tank knows about
    """
    ...
    def get_store_uri(self):
        """
        Returns the Tank image URI, which is the host:port of the API server
...
class StoreLocation(object):
    ...
    def get_uri(self):
        ...
        shall never be public and only used internally within Tank, so it is
