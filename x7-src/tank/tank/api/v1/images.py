...
"""
/images endpoint for Tank v1 API
"""
...
import tank.api.v1
from tank.api.v1 import controller
from tank import image_cache
from tank.common import cfg
from tank.common import exception
from tank.common import wsgi
from tank.common import utils
import tank.store
import tank.store.filesystem
import tank.store.http
import tank.store.rbd
import tank.store.s3
import tank.store.chase
from tank.store import (get_from_backend,
                          schedule_delete_from_backend,
                          get_store_from_location,
                          get_store_from_scheme,
                          UnsupportedBackend)
from tank import registry
from tank import notifier


logger = logging.getLogger(__name__)
SUPPORTED_PARAMS = tank.api.v1.SUPPORTED_PARAMS
SUPPORTED_FILTERS = tank.api.v1.SUPPORTED_FILTERS


class Controller(controller.BaseController):
    """
    WSGI controller for images resource in Tank v1 API
    ...
    def create(self, req, image_meta, image_data):
        """
        Adds a new image to Tank. Three scenarios exist when creating an
        image:
        ...
        3. If the image data is not available yet, but you'd like reserve a
           ...
           compatibility with X7/Rackspace API semantics.
    ...
    def delete(self, req, id):
        """
        Deletes the image and all its chunks from the Tank
        ...
        # See https://bugs.launchpad.net/tank/+bug/747799
        try:
            if image['location']:
    ...
    def get_store_or_400(self, request, store_name):
        ...
        try:
            ...
            msg = (_("Requested store %s not available on this Tank server")
                   % store_name)
            logger.error(msg)
