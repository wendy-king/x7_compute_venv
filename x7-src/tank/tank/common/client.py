...
class ImageBodyIterator(object):
    ...
    tuple from `tank.client.Client.get_image`
    """
...
class BaseClient(object):
    ...
    def __init__(self, host, port=None, use_ssl=False, auth_tok=None,
        ...
        if use_ssl:
            ...
            if not cert_file:
                if not os.environ.get('TANK_CLIENT_CERT_FILE'):
                    msg = _("You have selected to use SSL in connecting, "
                            "however you have failed to supply either a "
                            "cert_file parameter or set the "
                            "TANK_CLIENT_CERT_FILE environ variable")
                    raise exception.ClientConnectionError(msg)
                cert_file = os.environ.get('TANK_CLIENT_CERT_FILE')
            ...
            if not ca_file:
                ca_file = os.environ.get('TANK_CLIENT_CA_FILE')
    ...
    def set_auth_token(self, auth_tok):
        ...
        """
        # FIXME(sirp): Engine image/tank.py currently calls this. Since this
        # method isn't really doing anything useful[1], we should go ahead and
        # rip it out, first in Engine, then here. Steps:
        #
        #       1. Change auth_tok in Tank to auth_token
        #       2. Change image/tank.py in Engine to use client.auth_token
