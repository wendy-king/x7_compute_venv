...
class TestPrivateImagesCli(keystone_utils.KeystoneTests):
    ...
    def test_tank_cli(self):
        ...
        cmd = ("echo testdata | bin/tank --port=%d --auth_token=%s add "
               "name=MyImage" %
        ...
        cmd = "bin/tank --port=%d --auth_token=%s update %s is_public=True"
        ...
        cmd = "bin/tank --port=%d --auth_token=%s update %s owner=froggy"
        exitcode, out, err = execute(cmd % args)
        ...
        args = (self.api_port, keystone_utils.admin_token, image_id)
        cmd = "bin/tank --port=%d --auth_token=%s update %s owner="
        exitcode, out, err = execute(cmd % args)
