...
from tank.tests.functional import keystone_utils
...
class TestSharedImagesCli(keystone_utils.KeystoneTests):
    ...
    def test_share_image(self):
        ...
        cmd = "bin/tank --port=%d --auth_token=%s member-add %s %s" % args
        ...
        cmd = ("bin/tank --port=%d --auth_token=%s members-replace %s %s "
               "--can-share" %
        ...
        cmd = ("bin/tank --port=%d --auth_token=%s member-delete %s %s" %
               (self.api_port, keystone_utils.pattieblack_token,
                image_id, keystone_utils.bacon_id))
        exitcode, out, err = execute(cmd)
    ...
    def test_list_shares(self):
        ...
        cmd = ("bin/tank --port=%d --auth_token=%s member-add %s %s" %
               (self.api_port, keystone_utils.pattieblack_token,
        ...
        cmd = ("bin/tank --port=%d --auth_token=%s image-members %s" %
        ...
        cmd = ("bin/tank --port=%d --auth_token=%s member-images %s" %
               (self.api_port, keystone_utils.pattieblack_token,
                keystone_utils.froggy_id))
        exitcode, out, err = execute(cmd)
        self.assertEqual(0, exitcode)
