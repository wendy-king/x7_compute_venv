...
"""
Tests a Tank API server which uses an S3 backend by default

This test requires that a real S3 account is available. It looks
in a file specified in the TANK_TEST_S3_CONF environ variable
...
class TestS3(test_api.TestApi):
    ...
    # Test machines can set the TANK_TEST_S3_CONF variable
    # to override the location of the config file for S3 testing
    CONFIG_FILE_PATH = os.environ.get('TANK_TEST_S3_CONF')
    ...
    def setUp(self):
        ...
        if not self.CONFIG_FILE_PATH:
            self.disabled_message = "TANK_TEST_S3_CONF environ not set."
            self.inited = True
            return
