...
from tank.tests.utils import execute, find_executable
...
class KeystoneServer(functional.Server):
    ...
    def __init__(self, server_control, server_name, test_dir, port,
        ...
        self.sql_connection = os.environ.get('TANK_TEST_KEYSTONE_SQL',
...
service-header-mappings = {
        'engine' : 'X-Server-Management-Url',
        'chase' : 'X-Storage-Url',
...
def conf_patch(server, **subs):
    ...
    for idx, text in enumerate(conf_base):
        if text.startswith('[pipeline:tank-'):
...
tank.filter_factory =
 keystone.middleware.tank_auth_token:KeystoneContextMiddleware
""" % subs
...
class KeystoneTests(functional.FunctionalTest):
    ...
    def start_servers(self, **kwargs):
        ...
        keystone-admin) on unused ports, in addition to the Tank API
    ...
    def stop_servers(self):
        ...
        use the tank-control and keystone-control stop method to
