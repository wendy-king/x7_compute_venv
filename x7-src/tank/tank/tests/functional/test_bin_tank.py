...
"""Functional test case that utilizes the bin/tank CLI tool"""
...
class TestBinTank(functional.FunctionalTest):
    """Functional tests for the bin/tank CLI tool"""

    def setUp(self):
        super(TestBinTank, self).setUp()
    ...
    def test_add_with_location(self):
        ...
        cmd = "bin/tank --port=%d index" % api_port
        ...
        # 1. Add public image
        cmd = "bin/tank --port=%d add is_public=True"\
    ...
    def test_add_with_location_and_stdin(self):
        ...
        with tempfile.NamedTemporaryFile() as image_file:
            ...
            cmd = "bin/tank --port=%d add is_public=True name=MyImage " \
    ...
    def test_add_list_delete_list(self):
        ...
        # 3. Delete the image
        cmd = "bin/tank --port=%d --force delete %s" % (api_port, image_id)
    ...
    def test_add_list_update_list(self):
        ...
        # 1. Add public image
        cmd = "bin/tank --port=%d add name=MyImage" % api_port
        ...
        cmd = "bin/tank --port=%d update %s is_public=True" % (
            api_port, image_id)
        ...
        cmd = "bin/tank --port=%d update %s is_public=True name=\"%s\"" \
    ...
    def test_killed_image_not_in_index(self):
        ...
        in the 'killed' status is displayed in the output of tank index,
        and the status column is not displayed in the output of
        tank show <ID>.

            Start servers with Chase backend and a bad auth URL, and then:
            0. Verify no public images in index
        ...
        # Start servers with a Chase backend and a bad auth URL
        options = {'default_store': 'chase',
                   'chase_store_auth_address': 'badurl'}
        ...
        with tempfile.NamedTemporaryFile() as image_file:
            ...
            image_file.flush()
            image_file_name = image_file.name
            cmd = ("bin/tank --port=%d add name=Jonas is_public=True "
    ...
    def test_add_clear(self):
        ...
        # 1. Add some images
        for i in range(1, 5):
            cmd = "bin/tank --port=%d add is_public=True name=MyName " \
                  " foo=bar" % api_port
            exitcode, out, err = execute(cmd)
        ...
        # 2. Clear all images
        cmd = "bin/tank --port=%d --force clear" % api_port
        exitcode, out, err = execute(cmd)
        self.assertEqual(0, exitcode)
    ...
    def test_results_filtering(self):
        ...
        # 1. Add some images
        _add_cmd = "bin/tank --port=%d add is_public=True" % api_port
        _add_args = [
        ...
        _base_cmd = "bin/tank --port=%d" % api_port
    ...
    def test_results_pagination(self):
        ...
        _base_cmd = "bin/tank --port=%d" % self.api_port
        index_cmd = "%s index -f" % _base_cmd
        details_cmd = "%s details -f" % _base_cmd

        # 1. Add some images
        _add_cmd = "bin/tank --port=%d add is_public=True" % self.api_port
        _add_args = [
    ...
    def test_show_image_format(self):
        ...
        # 2. Verify image added as public image
        cmd = "bin/tank --port=%d show %s" % (api_port, image_id)
    ...
    def test_protected_image(self):
        ...
        # 1. Add public image
        cmd = "echo testdata | bin/tank --port=%d add is_public=True"\
        ...
        cmd = "bin/tank --port=%d --force update %s" \
