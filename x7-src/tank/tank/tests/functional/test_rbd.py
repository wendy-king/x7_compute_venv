...
"""
Tests a Tank API server which uses an RBD backend by default

This test has requires a Ceph cluster, optionally with
authentication. It looks in a file specified in the
TANK_TEST_RBD_CONF environment variable for RBD store settings.
...
from tank.store.rbd import DEFAULT_POOL, DEFAULT_CONFFILE, DEFAULT_USER
...
class TestRBD(test_api.TestApi):
    ...
    CONFIG_FILE_PATH = os.environ.get('TANK_TEST_RBD_CONF')

    def __init__(self, *args, **kwargs):
        super(TestRBD, self).__init__(*args, **kwargs)
        self.disabled = True
        if not self.CONFIG_FILE_PATH:
            self.disabled_message = "TANK_TEST_RBD_CONF environ not set."
            return
