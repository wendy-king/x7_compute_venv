...
from tank.tests.utils import execute, skip_if_disabled
...
class TestSSL(functional.FunctionalTest):
    ...
    def test_queued_process_flow(self):
        ...
        with Tank but does not immediately upload an image file.
    ...
    def test_version_variations(self):
        ...
        # Bug lp:803260  no Accept header causes a 500 in tank-api
        ...
        # 4. GET / with an Accept: application/vnd.x7.images-v1
        # Verify empty image list returned
        path = "https://%s:%d/images" % ("0.0.0.0", self.api_port)
        https = httplib2.Http(disable_ssl_certificate_validation=True)
        headers = {'Accept': 'application/vnd.x7.images-v1'}
        response, content = https.request(path, 'GET', headers=headers)
        self.assertEqual(response.status, 200)
        self.assertEqual(content, images_json)

        # 5. GET /images with a Accept: application/vnd.x7.compute-v1
        # header. Verify version choices returned. Verify message in API log
        # about unknown accept header.
        path = "https://%s:%d/images" % ("0.0.0.0", self.api_port)
        https = httplib2.Http(disable_ssl_certificate_validation=True)
        headers = {'Accept': 'application/vnd.x7.compute-v1'}
        response, content = https.request(path, 'GET', headers=headers)
        self.assertEqual(response.status, 300)
        self.assertEqual(content, versions_json)
        self.assertTrue('Unknown accept header'
        ...
        # 10. GET /versions with a Accept: application/vnd.x7.images-v1
        ...
        # 13. GET /images with a Accept: application/vnd.x7.compute-v2
        # header. Verify version choices returned. Verify message in API log
        # about unknown version in accept header.
        path = "https://%s:%d/images" % ("0.0.0.0", self.api_port)
        https = httplib2.Http(disable_ssl_certificate_validation=True)
        headers = {'Accept': 'application/vnd.x7.images-v2'}
        response, content = https.request(path, 'GET', headers=headers)
        self.assertEqual(response.status, 300)
        self.assertEqual(content, versions_json)
        self.assertTrue('Unknown accept header'
    ...
    def test_size_greater_2G_mysql(self):
        ...
        :see https://bugs.launchpad.net/tank/+bug/739433
        """
        ...
        # X-Image-Meta-Location attribute to make Tank forego
    ...
    def test_traceback_not_consumed(self):
        ...
        :see https://bugs.launchpad.net/tank/+bug/755912
        """
        self.cleanup()
        self.start_servers(**self.__dict__.copy())
