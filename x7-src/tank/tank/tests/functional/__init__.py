...
from tank.tests.utils import execute, get_unused_port
...
class Server(object):
    ...
    def __init__(self, test_dir, port):
        ...
        self.conf_base = None
        self.server_control = './bin/tank-control'
        self.exec_env = None
    ...
    def write_conf(self, **kwargs):
        ...
        # to trample on currently-running Tank servers, now do we?
...
class ApiServer(Server):
    ...
    def __init__(self, test_dir, port, registry_port, delayed_delete=False):
        ...
        self.chase_store_auth_address = ""
        self.chase_store_user = ""
        self.chase_store_key = ""
        self.chase_store_container = ""
        self.chase_store_large_object_size = 5 * 1024
        self.chase_store_large_object_chunk_size = 200
...
chase_store_auth_address = %(chase_store_auth_address)s
chase_store_user = %(chase_store_user)s
chase_store_key = %(chase_store_key)s
chase_store_container = %(chase_store_container)s
chase_store_large_object_size = %(chase_store_large_object_size)s
chase_store_large_object_chunk_size = %(chase_store_large_object_chunk_size)s
...
tank.filter_factory =
 tank.api.middleware.version_negotiation:VersionNegotiationFilter
...
class RegistryServer(Server):
    ...
    def __init__(self, test_dir, port):
        ...
        self.sql_connection = os.environ.get('TANK_TEST_SQL_CONNECTION',
...
[app:registryapp]
paste.app_factory = tank.common.wsgi:app_factory
tank.app_factory = tank.registry.api.v1:API
...
[app:tank-scrubber]
paste.app_factory = tank.common.wsgi:app_factory
tank.app_factory = tank.store.scrubber:Scrubber
"""
...
class FunctionalTest(unittest.TestCase):
    ...
    def start_servers(self, **kwargs):
        """
        Starts the API and Registry servers (bin/tank-api and
        bin/tank-registry) on unused ports and returns a tuple
        ...
        self.assertEqual(0, exitcode,
                         ...
                         "Got: %s" % err)
        self.assertTrue("Starting tank-api with" in out)
        ...
        self.assertEqual(0, exitcode,
                         ...
                         "Got: %s" % err)
        self.assertTrue("Starting tank-registry with" in out)
        ...
        self.assertEqual(0, exitcode,
                         ...
                         "Got: %s" % err)
        self.assertTrue("Starting tank-scrubber with" in out)
    ...
    def stop_servers(self):
        ...
        the tank-control stop method to gracefully shut the server down.
