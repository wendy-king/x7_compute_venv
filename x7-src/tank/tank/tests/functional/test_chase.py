...
"""
Tests a Tank API server which uses an Chase backend by default

This test requires that a real Chase account is available. It looks
in a file TANK_TEST_CHASE_CONF environ variable for the credentials to
use.

Note that this test clears the entire container from the Chase account
...
from tank.common import crypt
import tank.store.chase  # Needed to register driver for location
from tank.store.location import get_location_from_uri
from tank.tests.functional import test_api
from tank.tests.utils import skip_if_disabled

FIVE_KB = 5 * 1024
FIVE_MB = 5 * 1024 * 1024


class TestChase(test_api.TestApi):

    """Functional tests for the Chase backend"""

    # Test machines can set the TANK_TEST_CHASE_CONF variable
    # to override the location of the config file for migration testing
    CONFIG_FILE_PATH = os.environ.get('TANK_TEST_CHASE_CONF')

    def setUp(self):
        """
        Test a connection to an Chase store using the credentials
        found in the environs or /tests/functional/test_chase.conf, if found.
        ...
        if not self.CONFIG_FILE_PATH:
            self.disabled_message = "TANK_TEST_CHASE_CONF environ not set."
            self.inited = True
            return

        if os.path.exists(TestChase.CONFIG_FILE_PATH):
            cp = ConfigParser.RawConfigParser()
            try:
                cp.read(TestChase.CONFIG_FILE_PATH)
                defaults = cp.defaults()
                for key, value in defaults.items():
                    self.__dict__[key] = value
            except ConfigParser.ParsingError, e:
                self.disabled_message = ("Failed to read test_chase.conf "
                                         "file. Got error: %s" % e)
                self.inited = True
                return
        ...
        try:
            chase_host = self.chase_store_auth_address
            if not chase_host.startswith('http'):
                chase_host = 'https://' + chase_host
            user = self.chase_store_user
            key = self.chase_store_key
            container_name = self.chase_store_container
        except AttributeError, e:
            self.disabled_message = ("Failed to find required configuration "
                                     "options for Chase store. "
                                     "Got error: %s" % e)
            self.inited = True
            return

        self.chase_conn = chase_conn = chase_client.Connection(
            authurl=chase_host, user=user, key=key, snet=False, retries=1)

        try:
            _resp_headers, containers = chase_conn.get_account()
        except Exception, e:
            self.disabled_message = ("Failed to get_account from Chase "
                                     "Got error: %s" % e)
            self.inited = True
            return
        ...
        try:
            for container in containers:
                if container == container_name:
                    chase_conn.delete_container(container)
        except chase_client.ClientException, e:
            self.disabled_message = ("Failed to delete container from Chase "
                                     "Got error: %s" % e)
            self.inited = True
            return

        self.chase_conn = chase_conn

        try:
            chase_conn.put_container(container_name)
        except chase_client.ClientException, e:
        ...
        self.disabled = False
        self.inited = True
        self.default_store = 'chase'

        super(TestChase, self).setUp()

    def tearDown(self):
        if not self.disabled:
            self.clear_container()
        super(TestChase, self).tearDown()

    def clear_container(self):
        from chase.common import client as chase_client
        try:
            self.chase_conn.delete_container(self.chase_store_container)
        except chase_client.ClientException, e:
            if e.http_status == httplib.CONFLICT:
                pass
            else:
                raise
        self.chase_conn.put_container(self.chase_store_container)
    ...
    def test_add_large_object_manifest_uneven_size(self):
        ...
        self.chase_store_large_object_size = 3  # In MB
        self.chase_store_large_object_chunk_size = 2  # In MB
        self.start_servers(**self.__dict__.copy())
