...
While network traffic between VM instances belonging to the same VLAN is always open, Engine can enforce isolation of network traffic between different projects by enforcing one VLAN per project. 
...
For our implementation of Engine, our goal is that each project is in a protected network segment. Here are the specifications we keep in mind for meeting this goal.
...
Implementation
--------------
Currently Engine segregates project VLANs using 802.1q VLAN tagging in the 
...
The network assignment to a project, and IP address assignment to a VM instance, are triggered when a user starts to run a VM instance. When running a VM instance, a user needs to specify a project for the instances, and the security groups (described in Security Groups) when the instance wants to join. If this is the first instance to be created for the project, then Engine (the cloud controller) needs to find a network controller to be the network host for the project; it then sets up a private network by finding an unused VLAN id, an unused subnet, and then the controller assigns them to the project, it also assigns a name to the project's Linux bridge (br100 stored in the Engine database), and allocating a private IP within the project's subnet for the new instance.
...
Engine assumes the following is available:
...
* Assign a contiguous range of VLANs to Engine for project use.
...
* Configure Engine (need to add specifics here)
