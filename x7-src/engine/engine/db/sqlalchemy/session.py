...
import engine.log as logging


FLAGS = flags.FLAGS
LOG = logging.getLogger("engine.db.sqlalchemy.session")
...
def get_session(autocommit=True, expire_on_commit=False):
    ...
    session = _MAKER()
    session.query = engine.exception.wrap_db_error(session.query)
    session.flush = engine.exception.wrap_db_error(session.flush)
    return session
