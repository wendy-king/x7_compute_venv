...
#    under the License.
"""
SQLAlchemy models for engine data.
"""
...
class Service(BASE, EngineBase):
    ...
    availability_zone = Column(String(255), default='engine')


class ComputeNode(BASE, EngineBase):
...
class Certificate(BASE, EngineBase):
...
class Instance(BASE, EngineBase):
...
class InstanceInfoCache(BASE, EngineBase):
    """
...
class VirtualStorageArray(BASE, EngineBase):
    """
...
class InstanceActions(BASE, EngineBase):
...
class InstanceTypes(BASE, EngineBase):
...
class VolumeMetadata(BASE, EngineBase):
...
class VolumeTypes(BASE, EngineBase):
...
class VolumeTypeExtraSpecs(BASE, EngineBase):
...
class Snapshot(BASE, EngineBase):
    """Represents a block storage device that can be attached to a vm."""
...
class BlockDeviceMapping(BASE, EngineBase):
...
class IscsiTarget(BASE, EngineBase):
...
class SecurityGroupInstanceAssociation(BASE, EngineBase):
...
class SecurityGroup(BASE, EngineBase):
...
class SecurityGroupIngressRule(BASE, EngineBase):
...
class ProviderFirewallRule(BASE, EngineBase):
    """Represents a rule in a security group."""
...
class Migration(BASE, EngineBase):
...
class VirtualInterface(BASE, EngineBase):
...
class FloatingIp(BASE, EngineBase):
...
class AuthToken(BASE, EngineBase):
...
class UserProjectRoleAssociation(BASE, EngineBase):
...
class UserRoleAssociation(BASE, EngineBase):
...
class UserProjectAssociation(BASE, EngineBase):
...
class ConsolePool(BASE, EngineBase):
...
class InstanceMetadata(BASE, EngineBase):
...
class InstanceTypeExtraSpecs(BASE, EngineBase):
...
class AgentBuild(BASE, EngineBase):
...
class BandwidthUsage(BASE, EngineBase):
...
class S3Image(BASE, EngineBase):
    """Compatibility layer for the S3 image service talking to Tank"""
    __tablename__ = 's3_images'
    id = Column(Integer, primary_key=True, nullable=False, autoincrement=True)
    uuid = Column(String(36), nullable=False)


class SMFlavors(BASE, EngineBase):
...
class SMBackendConf(BASE, EngineBase):
...
class SMVolume(BASE, EngineBase):
...
class InstanceFault(BASE, EngineBase):
...
def register_models():
    ...
    Called from engine.db.sqlalchemy.__init__ as part of loading the driver,
