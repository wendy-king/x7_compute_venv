...
class TestLoaderNothingExists(unittest.TestCase):
    ...
    def test_config_not_found(self):
        self.assertRaises(
            engine.exception.ConfigNotFound,
            engine.wsgi.Loader,
        )
...
document_root = /tmp
    ...
    def setUp(self):
        ...
        self.config.seek(0)
        self.config.flush()
        self.loader = engine.wsgi.Loader(self.config.name)
    ...
    def test_app_not_found(self):
        self.assertRaises(
            engine.exception.PasteAppNotFound,
...
class TestWSGIServer(unittest.TestCase):
    ...
    def test_no_app(self):
        server = engine.wsgi.Server("test_app", None)
        self.assertEquals("test_app", server.name)

    def test_start_random_port(self):
        server = engine.wsgi.Server("test_random_port", None, host="127.0.0.1")
