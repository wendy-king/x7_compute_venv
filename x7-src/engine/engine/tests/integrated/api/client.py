...
LOG = logging.getLogger('engine.tests.api')


class X7ApiException(Exception):
    def __init__(self, message=None, response=None):
        ...
        super(X7ApiException, self).__init__(message)


class X7ApiAuthenticationException(X7ApiException):
    def __init__(self, response=None, message=None):
        if not message:
            message = _("Authentication error")
        super(X7ApiAuthenticationException, self).__init__(message,
                                                                  response)


class X7ApiAuthorizationException(X7ApiException):
    def __init__(self, response=None, message=None):
        if not message:
            message = _("Authorization error")
        super(X7ApiAuthorizationException, self).__init__(message,
                                                                  response)


class X7ApiNotFoundException(X7ApiException):
    def __init__(self, response=None, message=None):
        if not message:
            message = _("Item not found")
        super(X7ApiNotFoundException, self).__init__(message, response)


class TestX7Client(object):
    """Simple X7 API Client.

    This is a really basic X7 API client that is under our control,
    so we can make changes / insert hooks for testing

    """

    def __init__(self, auth_user, auth_key, auth_uri):
        super(TestX7Client, self).__init__()
    ...
    def request(self, url, method='GET', body=None, headers=None):
        ...
        if scheme == 'http':
            ...
            raise X7ApiException("Unknown scheme: %s" % url)
    ...
    def _authenticate(self):
        ...
        if http_status == 401:
            raise X7ApiAuthenticationException(response=response)
    ...
    def api_request(self, relative_uri, check_response_status=None, **kwargs):
        ...
        if check_response_status:
            if not http_status in check_response_status:
                if http_status == 404:
                    raise X7ApiNotFoundException(response=response)
                elif http_status == 401:
                    raise X7ApiAuthorizationException(response=response)
                else:
                    raise X7ApiException(
