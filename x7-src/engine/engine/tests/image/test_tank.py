...
class TestTankSerializer(test.TestCase):
    def test_serialize(self):
        ...
        converted_expected = {
            ...
            'properties': {
                ...
                '{"virtual_device": "ephemeral0", '
                '"device_name": "/dev/fake0"}]'}}
        converted = tank._convert_to_string(metadata)
        self.assertEqual(converted, converted_expected)
        self.assertEqual(tank._convert_from_string(converted), metadata)


class TestTankImageService(test.TestCase):
    """
    Tests the Tank image service.

    At a high level, the translations involved are:

        1. Tank -> ImageService - This is needed so we can support
           multple ImageServices (Tank, Local, etc)

        2. ImageService -> API - This is needed so we can support multple
           APIs (X7, EC2)

    """
    NOW_TANK_OLD_FORMAT = "2010-10-11T10:30:22"
    NOW_TANK_FORMAT = "2010-10-11T10:30:22.000000"
    ...
    def tearDown(self):
        self.stubs.UnsetAll()
        super(TestTankImageService, self).tearDown()
    ...
    def _make_datetime_fixture(self):
        return self._make_fixture(created_at=self.NOW_TANK_FORMAT,
                                  updated_at=self.NOW_TANK_FORMAT,
                                  deleted_at=self.NOW_TANK_FORMAT)
    ...
    def test_get_with_retries(self):
        ...
        class TankBusyException(Exception):
            pass

        class MyTankStubClient(tank_stubs.StubTankClient):
            """A client that fails the first time, then succeeds."""
            def get_image(self, image_id):
                if tries[0] == 0:
                    tries[0] = 1
                    raise TankBusyException()
                else:
                    return {}, []

        client = MyTankStubClient()
        service = tank.TankImageService(client=client)
        image_id = 1  # doesn't matter
        writer = NullWriter()

        # When retries are disabled, we should get an exception
        self.flags(tank_num_retries=0)
        self.assertRaises(TankBusyException, service.get, self.context,
                          image_id, writer)

        # Now lets enable retries. No exception should happen now.
        self.flags(tank_num_retries=1)
