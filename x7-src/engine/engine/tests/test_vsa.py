...
from engine.vsa import utils as vsa_utils

import engine.image.fake

FLAGS = flags.FLAGS
LOG = logging.getLogger('engine.tests.vsa')
...
class VsaTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stubs.Set(engine.image.fake._FakeImageService,
                        'show_by_name',
                        fake_show_by_name)
    ...
    def test_vsa_create_db_error(self):
        ...
        self.stubs.Set(engine.db, 'vsa_create', fake_vsa_create)
        self.assertRaises(exception.ApiError,
