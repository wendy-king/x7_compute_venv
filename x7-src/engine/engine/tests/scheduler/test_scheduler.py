...
from engine.scheduler.simple import SimpleScheduler
...
class SchedulerTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        driver = 'engine.tests.scheduler.test_scheduler.TestDriver'
    ...
    def _create_compute_service(self):
        ...
        ctxt = context.get_admin_context()
        dic = {'host': 'dummy', 'binary': 'engine-compute', 'topic': 'compute',
...
class SimpleDriverTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        simple_scheduler = 'engine.scheduler.simple.SimpleScheduler'
        self.flags(connection_type='fake',
                stub_network=True,
                max_cores=4,
                max_gigabytes=4,
                network_manager='engine.network.manager.FlatManager',
                volume_driver='engine.volume.driver.FakeISCSIDriver',
                scheduler_driver='engine.scheduler.multi.MultiScheduler',
    ...
    def _create_compute_service(self, **kwargs):
        ...
        dic = {'binary': 'engine-compute', 'topic': 'compute',
               'report_count': 0, 'availability_zone': 'dummyzone'}
    ...
    def test_doesnt_report_disabled_hosts_as_up_no_queue(self):
        ...
        compute2 = service.Service('host2',
                                   ...
                                   FLAGS.compute_manager)
        compute2.start()
        s1 = db.service_get_by_args(self.context, 'host1', 'engine-compute')
        s2 = db.service_get_by_args(self.context, 'host2', 'engine-compute')
    ...
    def test_specific_host_gets_instance_no_queue(self):
        ...
        request_spec = _create_request_spec(availability_zone='engine:host1')
    ...
    def test_live_migration_src_check_volume_node_not_alive(self):
        ...
        dic = {'created_at': t1, 'updated_at': t1, 'binary': 'engine-volume',
...
class MultiDriverTestCase(SimpleDriverTestCase):
    ...
    def setUp(self):
        ...
        self.flags(connection_type='fake',
                   ...
                   network_manager='engine.network.manager.FlatManager',
                   volume_driver='engine.volume.driver.FakeISCSIDriver',
                   compute_scheduler_driver=('engine.scheduler.simple'
                                             '.SimpleScheduler'),
                   volume_scheduler_driver=('engine.scheduler.simple'
                                            '.SimpleScheduler'),
                   scheduler_driver='engine.scheduler.multi.MultiScheduler')
        self.scheduler = manager.SchedulerManager()
...
class ZoneRedirectTest(test.TestCase):
    ...
    def test_one_zone_down_no_instances(self):
        ...
        def _fake_issue_engineclient_command(engine, zone, *args, **kwargs):
            return None

        class FakeEngineClientWithFailure(object):
            ...
            def authenticate(self):
                if self.api_url == ZONE_API_URL2:
                    raise engineclient_exceptions.BadRequest('foo')

        self.stubs.Set(api, '_issue_engineclient_command',
                _fake_issue_engineclient_command)
        self.stubs.Set(api.engineclient, 'Client', FakeEngineClientWithFailure)
    ...
    def test_zones_up_no_instances(self):
        ...
        class FakeEngineClientNoFailure(object):
            def __init__(self, username, password, method, api_url,
                         token=None, region_name=None):
                pass
        ...
        self.stubs.Set(api, '_issue_engineclient_command',
                _fake_issue_engineclient_command)
        self.stubs.Set(api.engineclient, 'Client', FakeEngineClientNoFailure)
...
class FakeEmptyServerCollection(object):
    def get(self, f):
        raise engineclient_exceptions.NotFound(1)

    def find(self, name):
        raise engineclient_exceptions.NotFound(2)
...
class DynamicEngineClientTest(test.TestCase):
    def test_issue_engineclient_command_found(self):
        zone = FakeZone(1, 'http://example.com', 'bob', 'xxx')
        self.assertEquals(api._issue_engineclient_command(
                    FakeEngineClient(FakeServerCollection()),
    ...
    def test_issue_engineclient_command_not_found(self):
        zone = FakeZone(1, 'http://example.com', 'bob', 'xxx')
        try:
            api._issue_engineclient_command(FakeEngineClient(
...
class FakeEngineClientZones(object):
    def __init__(self, *args, **kwargs):
...
class CallZoneMethodTest(test.TestCase):
    def setUp(self):
        ...
        self.stubs.Set(db, 'zone_get_all', zone_get_all)
        self.stubs.Set(engineclient, 'Client', FakeEngineClientZones)
