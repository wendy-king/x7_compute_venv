...
from engine.scheduler import distributed_scheduler as dist
from engine.scheduler import zone_manager
from engine.tests.scheduler import fake_zone_manager as ds_fakes
...
class HostFilterTestCase(test.TestCase):
    ...
    def test_instance_type_filter(self):
        hf = engine.scheduler.filters.InstanceTypeFilter()
    ...
    def test_json_filter(self):
        hf = engine.scheduler.filters.JsonFilter()
        # filter all hosts that can support 30 ram and 300 disk
        cooked = hf.instance_type_to_filter(self.instance_type)
        all_hosts = self._get_all_hosts()
        hosts = hf.filter_hosts(all_hosts, cooked, {})
