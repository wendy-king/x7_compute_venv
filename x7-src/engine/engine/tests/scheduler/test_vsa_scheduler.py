...
from engine.scheduler import vsa as vsa_sched

FLAGS = flags.FLAGS
LOG = logging.getLogger('engine.tests.scheduler.vsa')
...
class VsaSchedulerTestCase(test.TestCase):
    ...
    def setUp(self, sched_class=None):
        ...
        self.stubs.Set(engine.db, 'vsa_update', self._fake_vsa_update)

        self.stubs.Set(engine.db, 'volume_get', self._fake_volume_get)
        self.stubs.Set(engine.db, 'volume_update', self._fake_volume_update)
    ...
    def test_vsa_sched_provision_volume(self):
        ...
        self.stubs.Set(self.sched,
                        '_get_service_states', self._fake_get_service_states)
        self.stubs.Set(engine.db, 'volume_create', self._fake_volume_create)
        self.stubs.Set(engine.db, 'volume_update', self._fake_volume_update)
        self.stubs.Set(rpc, 'cast', fake_rpc_cast)
    ...
    def test_vsa_sched_forced_host(self):
        ...
        self.assertRaises(exception.HostBinaryNotFound,
                          self.sched.schedule_create_volumes,
                                self.context,
                                request_spec,
                                availability_zone="engine:host_5")
    ...
    def test_vsa_sched_create_single_volume_az(self):
        ...
        def _fake_volume_get_az(context, volume_id):
            ...
            return {'id': volume_id, 'availability_zone': 'engine:host_3'}

        self.stubs.Set(engine.db, 'volume_get', _fake_volume_get_az)
        self.stubs.Set(engine.db,
                        'service_get_by_args', self._fake_service_get_by_args)
        self.stubs.Set(self.sched,
                        'service_is_up', self._fake_service_is_up_True)
