...
def stubout_tank_client(stubs):
    def fake_get_tank_client(context, image_href):
        image_id = int(str(image_href).split('/')[-1])
        return (FakeTank('foo'), image_id)
    stubs.Set(tank, 'get_tank_client', fake_get_tank_client)
...
class StubTankClient(object):
    ...
    def add_image(self, metadata, data):
        metadata['created_at'] = NOW_TANK_FORMAT
        metadata['updated_at'] = NOW_TANK_FORMAT
