...
class _VirtDriverTestCase(test.TestCase):
    ...
    def test_agent_update(self):
        instance_ref, network_info = self._get_running_instance()
        self.connection.agent_update(instance_ref, 'http://www.x7.org/',
...
class AbstractDriverTestCase(_VirtDriverTestCase):
    def setUp(self):
        import engine.virt.driver

        self.driver_module = engine.virt.driver

        def get_driver_connection(_):
            return engine.virt.driver.ComputeDriver()
...
class FakeConnectionTestCase(_VirtDriverTestCase):
    def setUp(self):
        import engine.virt.fake
        self.driver_module = engine.virt.fake
...
class LibvirtConnTestCase(_VirtDriverTestCase):
    def setUp(self):
        ...
        import engine.virt.libvirt.connection
        import engine.virt.libvirt.firewall

        engine.virt.libvirt.connection.libvirt = fakelibvirt
        engine.virt.libvirt.connection.libvirt_utils = fake_libvirt_utils
        engine.virt.libvirt.firewall.libvirt = fakelibvirt

        # Point _VirtDriverTestCase at the right module
        self.driver_module = engine.virt.libvirt.connection
    ...
    def tearDown(self):
        ...
        if self.saved_libvirt:
            ...
            engine.virt.libvirt.connection.libvirt = self.saved_libvirt
            engine.virt.libvirt.connection.libvirt_utils = self.saved_libvirt
            engine.virt.libvirt.firewall.libvirt = self.saved_libvirt
