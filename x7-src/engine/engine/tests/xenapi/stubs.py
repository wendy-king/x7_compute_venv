...
from engine.virt.xenapi import fake
...
def stubout_session(stubs, cls, product_version=None):
    ...
    def fake_import(self):
        ...
        fake_module = 'engine.virt.xenapi.fake'
        from_list = ['fake']
...
def stubout_fetch_image_tank_disk(stubs):
    """Simulates a failure in fetch image_tank_disk."""

    @classmethod
    def f(cls, *args):
        raise fake.Failure("Test Exception raised by " +
                           "fake fetch_image_tank_disk")
    stubs.Set(vm_utils.VMHelper, '_fetch_image_tank_disk', f)
...
class FakeSessionForVMTests(fake.SessionBase):
    ...
    def host_call_plugin(self, _1, _2, plugin, method, _5):
        if (plugin, method) == ('tank', 'download_vhd'):
