...
from engine.network.quantum import quantum_connection
from engine.network.quantum import melange_ipam_lib
from engine import utils

LOG = logging.getLogger("engine.network.quantum.manager")

FLAGS = flags.FLAGS

flags.DEFINE_string('quantum_ipam_lib',
                    'engine.network.quantum.engine_ipam_lib',
...
class QuantumManager(manager.FlatManager):
    """NetworkManager class that communicates with a Quantum service
       ...
       use either Engine's local DB or the Melange IPAM service.
    ...
    def _get_engine_id(self, context):
