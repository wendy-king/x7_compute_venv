...
LOG = logging.getLogger("engine.linux_net")
...
flags.DEFINE_string('dhcpbridge_flagfile',
                    '/etc/engine/engine-dhcpbridge.conf',
...
flags.DEFINE_string('dhcpbridge', _bin_file('engine-dhcpbridge'),
                        'location of engine-dhcpbridge')
...
class IptablesTable(object):
    ...
    def add_chain(self, name, wrap=True):
        ...
        it, so different components of Engine can safely create identically
        named chains without interfering with one another.

        At the moment, its wrapped name is <binary name>-<chain name>,
        so if engine-compute creates a chain named 'OUTPUT', it'll actually
        end up named 'engine-compute-OUTPUT'.
...
class IptablesManager(object):
    ...
    First, engine-filter-top. It's added at the top of FORWARD and OUTPUT. Its
    name is not wrapped, so it's shared between the various engine workers. It's
    ...
    "local" which is jumped to from engine-filter-top.
    ...
    def __init__(self, execute=None):
        ...
        # Add a engine-filter-top chain. It's intended to be shared
        # among the various engine components. It sits at the very top
        # of FORWARD and OUTPUT.
        for tables in [self.ipv4, self.ipv6]:
            tables['filter'].add_chain('engine-filter-top', wrap=False)
            tables['filter'].add_rule('FORWARD', '-j engine-filter-top',
                                      wrap=False, top=True)
            tables['filter'].add_rule('OUTPUT', '-j engine-filter-top',
                                      wrap=False, top=True)

            tables['filter'].add_chain('local')
            tables['filter'].add_rule('engine-filter-top', '-j $local',
                                      wrap=False)
        ...
        # Add a engine-postrouting-bottom chain. It's intended to be shared
        # among the various engine components. We set it as the last chain
        # of POSTROUTING chain.
        self.ipv4['nat'].add_chain('engine-postrouting-bottom', wrap=False)
        self.ipv4['nat'].add_rule('POSTROUTING', '-j engine-postrouting-bottom',
                                  wrap=False)

        # We add a snat chain to the shared engine-postrouting-bottom chain
        # so that it's applied last.
        self.ipv4['nat'].add_chain('snat')
        self.ipv4['nat'].add_rule('engine-postrouting-bottom', '-j $snat',
                                  wrap=False)
    ...
    def apply(self):
        ...
        same component of Engine, and replace them with our current set of
...
def _dhcp_file(dev, kind):
    ...
    return os.path.abspath('%s/engine-%s.%s' % (FLAGS.networks_path,
                                              dev,
                                              kind))
...
def _ra_file(dev, kind):
    ...
    ensure_path(FLAGS.networks_path)
    return os.path.abspath('%s/engine-ra-%s.%s' % (FLAGS.networks_path,
                                              dev,
                                              kind))
