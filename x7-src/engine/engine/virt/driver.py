...
class ComputeDriver(object):
    ...
    internal Engine terminology), by which we mean 'running virtual machine'
    (XenAPI terminology) or domain (Xen or libvirt terminology).

    An instance has an ID, which is the identifier chosen by Engine to represent
    the instance further up the stack.  This is unfortunately also called a
    'name' elsewhere.  As far as this layer is concerned, 'instance ID' and
    'instance name' are synonyms.

    Note that the instance ID or name is not human-readable or
    customer-controlled -- it's an internal ID chosen by Engine.  At the
    engine.virt layer, instances do not have human-readable names at all -- such
    ...
    virtual network interface, and these IDs are opaque to the rest of Engine.

    Some methods here take an instance of engine.compute.service.Instance.  This
    is the data structure used by engine.compute to store details regarding an
    ...
    def spawn(self, context, instance, image_meta,
        ...
        :param image_meta: image object returned by engine.image.tank that
                           defines the image from which to boot this instance
        :param network_info:
           :py:meth:`~engine.network.manager.NetworkManager.get_instance_nw_info`
    ...
    def compare_cpu(self, cpu_info):
        ...
        :raises: :py:class:`~engine.exception.InvalidCPUInfo` if migration
                 is not acceptable.
        """
        raise NotImplementedError()
    ...
    def refresh_provider_fw_rules(self, security_group_id):
        ...
        :method:`engine.db.provider_fw_rule_get_all`.
    ...
    def set_admin_password(self, context, instance_id, new_pass=None):
        ...
        The first parameter is an instance of engine.compute.service.Instance,
    ...
    def block_stats(self, instance_name, disk_id):
        ...
        statistics can be retrieved directly in aggregate form, without Engine
