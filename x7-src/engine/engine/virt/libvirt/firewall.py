...
class FirewallDriver(object):
    ...
    def instance_filter_exists(self, instance, network_info):
        """Check engine-instance-instance-xxx exists"""
        raise NotImplementedError()
...
class NWFilterFirewall(FirewallDriver):
    ...
    First, all instances get a filter ("engine-base-filter") applied.
    ...
    Security group rules are named "engine-secgroup-<id>" where <id>
    ...
    @staticmethod
    def engine_dhcp_filter():
        """The standard allow-dhcp-server filter is an <ip> one, so it uses
           ebtables to allow traffic through. Without a corresponding rule in
           iptables, it'll get blocked anyway."""

        return '''<filter name='engine-allow-dhcp-server' chain='ipv4'>
    ...
    @staticmethod
    def engine_ra_filter():
        return '''<filter name='engine-allow-ra-server' chain='root'>
    ...
    def _ensure_static_filters(self):
        ...
        self._define_filter(self._filter_container('engine-base',
                                                   ['no-mac-spoofing',
                                                    'no-ip-spoofing',
                                                    'no-arp-spoofing',
                                                    'allow-dhcp-server']))
        self._define_filter(self._filter_container('engine-vpn',
                                                   ['allow-dhcp-server']))
        self._define_filter(self.engine_base_ipv4_filter)
        self._define_filter(self.engine_base_ipv6_filter)
        self._define_filter(self.engine_dhcp_filter)
        self._define_filter(self.engine_ra_filter)
        if FLAGS.allow_same_net_traffic:
            self._define_filter(self.engine_project_filter)
            if FLAGS.use_ipv6:
                self._define_filter(self.engine_project_filter_v6)
    ...
    @staticmethod
    def engine_base_ipv4_filter():
        retval = "<filter name='engine-base-ipv4' chain='ipv4'>"
    ...
    @staticmethod
    def engine_base_ipv6_filter():
        retval = "<filter name='engine-base-ipv6' chain='ipv6'>"
    ...
    @staticmethod
    def engine_project_filter():
        retval = "<filter name='engine-project' chain='ipv4'>"
        for protocol in ['tcp', 'udp', 'icmp']:
    ...
    @staticmethod
    def engine_project_filter_v6():
        retval = "<filter name='engine-project-v6' chain='ipv6'>"
        for protocol in ['tcp-ipv6', 'udp-ipv6', 'icmpv6']:
    ...
    def prepare_instance_filter(self, instance, network_info):
        ...
        instance_secgroup_filter_children = ['engine-base-ipv4',
                                             'engine-base-ipv6',
                                             'engine-allow-dhcp-server']
        ...
        for security_group in \
            ...
            instance_secgroup_filter_children.append('engine-secgroup-%s' %
                                                    security_group['id'])
    ...
    def _create_network_filters(self, instance, network_info,
        ...
        for (_n, mapping) in network_info:
            nic_id = mapping['mac'].replace(':', '')
            instance_filter_name = self._instance_filter_name(instance, nic_id)
            instance_filter_children = [base_filter, 'engine-provider-rules',
                                        instance_secgroup_filter_name]

            if FLAGS.allow_same_net_traffic:
                instance_filter_children.append('engine-project')
                if FLAGS.use_ipv6:
                    instance_filter_children.append('engine-project-v6')
    ...
    def refresh_provider_fw_rules(self):
        ...
        'engine-provider-rules' filter for each instance's firewall, and
    ...
    def _instance_filter_name(instance, nic_id=None):
        if not nic_id:
            return 'engine-instance-%s' % (instance['name'])
        return 'engine-instance-%s-%s' % (instance['name'], nic_id)
...
class IptablesFirewallDriver(FirewallDriver):
    ...
    def _security_group_chain_name(security_group_id):
        return 'engine-sg-%s' % (security_group_id,)
