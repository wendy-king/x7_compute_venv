...
from engine.virt.xenapi import HelperBase
from engine.virt.xenapi import volume_utils


LOG = logging.getLogger("engine.virt.xenapi.vm_utils")
...
class VMHelper(HelperBase):
    ...
    def upload_image(cls, context, session, instance, vdi_uuids, image_id):
        """ Requests that the Tank plugin bundle the specified VDIs and
        push them into Tank using the specified human-friendly name.
        """
        ...
        tank_host, tank_port = tank.pick_tank_api_server()
        ...
        task = session.async_call_plugin('tank', 'upload_vhd', kwargs)
    ...
    def fetch_image(cls, context, session, instance, image, user_id,
        ...
        """Fetch image from tank based on image type.

        Returns: A single filename if image_type is KERNEL or RAMDISK
                 A list of dictionaries that describe VDIs, otherwise
        """
        if image_type == ImageType.DISK_VHD:
            return cls._fetch_image_tank_vhd(context,
                session, instance, image, image_type)
        else:
            return cls._fetch_image_tank_disk(context,
                session, instance, image, image_type)

    @classmethod
    def _fetch_image_tank_vhd(cls, context, session, instance, image,
                                image_type):
        """Tell tank to download an image and put the VHDs into the SR
        ...
        # NOTE(sirp): The Tank plugin runs under Python 2.4
        ...
        kwargs = {'params': pickle.dumps(params)}
        task = session.async_call_plugin('tank', 'download_vhd', kwargs)
    ...
    @classmethod
    def _fetch_image_tank_disk(cls, context, session, instance, image,
                                 image_type):
        """Fetch the image from Tank

        NOTE:
        Unlike _fetch_image_tank_vhd, this method does not use the Tank
        ...
        """
        instance_id = instance.id
        # FIXME(sirp): Since the Tank plugin seems to be required for the
        ...
        tank_client.set_auth_token(getattr(context, 'auth_token', None))
        meta, image_file = tank_client.get_image(image_id)
        ...
        try:
            ...
            if image_type in (ImageType.KERNEL, ImageType.RAMDISK):
                ...
                task = session.async_call_plugin('tank', fn, args)
            ...
            LOG.exception(_("instance %s: Failed to fetch tank image"),
    ...
    def determine_disk_image_type(cls, image_meta):
        ...
        1. If we're using Tank, we can use the image_type field to
           determine the image_type

        2. If we're not using Tank, then we need to deduce this based on
        ...
        def determine_from_image_meta():
            ...
            try:
                return tank_disk_format2engine_type[disk_format]
            except KeyError:
    ...
    def determine_is_pv(cls, session, instance_id, vdi_ref, disk_image_type,
        ...
        will use hardware virtualization.
            ...
            1. Tank (VHD): then we use `os_type`, raise if not set

            2. Tank (DISK_RAW): use Pygrub to figure out if pv kernel is
               available

            3. Tank (DISK): pv is assumed

            4. Tank (DISK_ISO): no pv is assumed
        """
