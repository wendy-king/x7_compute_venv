...
LOG = logging.getLogger("engine.virt.vmwareapi.io_util")

IO_THREAD_SLEEP_TIME = .01
TANK_POLL_INTERVAL = 5
...
class TankWriteThread(object):
    """Ensures that image data is written to in the tank client and that
    it is in correct ('active')state."""

    def __init__(self, input, tank_client, image_id, image_meta=None):
        if not image_meta:
            image_meta = {}
    ...
    def start(self):
        ...
        def _inner():
            ...
            self.tank_client.update_image(self.image_id,
            ...
            while self._running:
                try:
                    image_status = \
                        self.tank_client.get_image_meta(self.image_id).get(
                                                                "status")
                    ...
                    # If the state is killed, then raise an exception.
                        ...
                        self.stop()
                        exc_msg = _("Tank image %s is in killed state") %\
                                    self.image_id
                        LOG.exception(exc_msg)
                        self.done.send_exception(exception.Error(exc_msg))
                    elif image_status in ["saving", "queued"]:
                        greenthread.sleep(TANK_POLL_INTERVAL)
                    else:
                        self.stop()
                        exc_msg = _("Tank image "
