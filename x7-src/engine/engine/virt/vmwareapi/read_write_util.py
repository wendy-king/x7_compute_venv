...
(like Tank image storage and retrieval service) from/to ESX/ESXi server.
...
from tank import client

from engine import flags
from engine import log as logging

LOG = logging.getLogger("engine.virt.vmwareapi.read_write_util")

FLAGS = flags.FLAGS

USER_AGENT = "X7-ESX-Adapter"
...
class TankFileRead(object):
    """Tank file read handler class."""

    def __init__(self, tank_read_iter):
        self.tank_read_iter = tank_read_iter
    ...
    def get_next(self):
        ...
        for data in self.tank_read_iter:
            yield data
...
class VmWareHTTPReadFile(VMwareHTTPFile):
    ...
    def read(self, chunk_size):
        ...
        # data items of the chunk-size that Tank Client uses for read
        # while writing.
