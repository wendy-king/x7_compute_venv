...
LOG = logging.getLogger("engine.image.s3")
FLAGS = flags.FLAGS
...
class S3ImageService(object):
    ...
    def get_image_uuid(self, context, image_id):
        return engine.db.api.s3_image_get(context, image_id)['uuid']

    def get_image_id(self, context, image_uuid):
        return engine.db.api.s3_image_get_by_uuid(context, image_uuid)['id']

    def _create_image_id(self, context, image_uuid):
        return engine.db.api.s3_image_create(context, image_uuid)['id']
