...
"""Implementation of an image service that uses Tank as the backend"""
...
def _create_tank_client(context, host, port):
    if context.strategy == 'keystone':
        # NOTE(dprince): Tank client just needs auth_tok right? Should we
        # add username and tenant to the creds below?
        creds = {'strategy': 'keystone',
                 'username': context.user_id,
                 'tenant': context.project_id}
        tank_client = TankClient(host, port, auth_tok=context.auth_token,
                                     creds=creds)
    else:
        tank_client = TankClient(host, port)
    return tank_client


def pick_tank_api_server():
    """Return which Tank API server to use for the request
    ...
    one IP and route that to a real load-balancer.
        ...
        Returns (host, port)
    """
    host_port = random.choice(FLAGS.tank_api_servers)
    host, port_str = host_port.split(':')
    port = int(port_str)
    return host, port


def get_tank_client(context, image_href):
    """Get the correct tank client and id for the given image_href.
    ...
    :returns: a tuple of the form (tank_client, image_id)

    """
    tank_host, tank_port = pick_tank_api_server()

    # check if this is an id
    if '/' not in str(image_href):
        tank_client = _create_tank_client(context,
                                              tank_host,
                                              tank_port)
        return (tank_client, image_href)
        ...
        tank_client = _create_tank_client(context,
                                              tank_host,
                                              tank_port)
        return (tank_client, image_id)


class TankImageService(object):
    """Provides storage and retrieval of disk image objects within Tank."""

    def __init__(self, client=None):
        self._client = client

    def _get_client(self, context):
        # NOTE(sirp): we want to load balance each request across tank
        # servers. Since TankImageService is a long-lived object, `client`
        # is made to choose a new server each time via this property.
        if self._client is not None:
            return self._client
        tank_host, tank_port = pick_tank_api_server()
        return _create_tank_client(context, tank_host, tank_port)

    def index(self, context, **kwargs):
        """Calls out to Tank for a list of images available."""
    ...
    def detail(self, context, **kwargs):
        """Calls out to Tank for a list of detailed image information."""
        params = self._extract_query_params(kwargs)
        image_metas = self._get_images(context, **params)
        ...
        for image_meta in image_metas:
            if self._is_image_available(context, image_meta):
                base_image_meta = self._translate_from_tank(image_meta)
    ...
    def _fetch_images(self, fetch_func, **kwargs):
        """Paginate through results from tank server"""
    ...
    def show(self, context, image_id):
        ...
        except tank_exception.NotFound:
    ...
    def show_by_name(self, context, name):
        ...
        # TODO(vish): replace this with more efficient call when tank
        #             supports it.
    ...
    def get(self, context, image_id, data):
        """Calls out to Tank for metadata and data and writes data."""
        num_retries = FLAGS.tank_num_retries
    ...
    def create(self, context, image_meta, data=None):
        ...
        LOG.debug(_('Creating image in Tank. Metadata passed in %s'),
                  image_meta)
        sent_service_image_meta = self._translate_to_tank(image_meta)
        LOG.debug(_('Metadata after formatting for Tank %s'),
        ...
        base_image_meta = self._translate_from_tank(recv_service_image_meta)
        LOG.debug(_('Metadata returned from Tank formatted for Base %s'),
                  base_image_meta)
        return base_image_meta
    ...
    def update(self, context, image_id, image_meta, data=None):
        ...
        image_meta = self._translate_to_tank(image_meta)
        try:
            client = self._get_client(context)
    ...
    @classmethod
    def _translate_to_tank(cls, image_meta):
        image_meta = _convert_to_string(image_meta)
        image_meta = _remove_read_only(image_meta)
        return image_meta

    @classmethod
    def _translate_from_tank(cls, image_meta):
    ...
    def _is_image_available(context, image_meta):
        ...
        Under Tank, images are always available if the context has
        an auth_token.
...
def _convert_timestamps_to_datetimes(image_meta):
    ...
    for attr in ['created_at', 'updated_at', 'deleted_at']:
        if image_meta.get(attr):
            image_meta[attr] = _parse_tank_iso8601_timestamp(
                image_meta[attr])
    return image_meta


def _parse_tank_iso8601_timestamp(timestamp):
...
# TODO(yamahata): use block-device-mapping extension to tank
