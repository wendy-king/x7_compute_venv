...
"""Engine base exception handling.

Includes decorator for re-raising Engine-type exceptions.
...
import engineclient.exceptions
import webob.exc
...
def engineclient_converter(f):
    """Convert engineclient ClientException HTTP codes to webob exceptions.
    ...
    def new_f(*args, **kwargs):
        try:
            ret = f(*args, **kwargs)
            return ret
        except engineclient.exceptions.ClientException, e:
...
def wrap_exception(notifier=None, publisher_id=None, event_type=None,
    ...
    """
    # TODO(sandy): Find a way to import engine.notifier.api so we don't have
    # to pass it in as a parameter. Otherwise we get a cyclic import of
    # engine.notifier.api -> engine.utils -> engine.exception :(
...
class ImagePaginationFailed(EngineException):
    message = _("Failed to paginate through images from image service")


class VirtualInterfaceCreateException(EngineException):
    message = _("Virtual Interface creation failed")


class VirtualInterfaceMacAddressException(EngineException):
    message = _("5 attempts to create virtual interface"
                "with unique mac address failed")


class NotAuthorized(EngineException):
...
class InstanceBusy(EngineException):
...
class VolumeIsBusy(EngineException):
...
class NetworkNotCreated(EngineException):
...
class NetworkHostNotSet(EngineException):
...
class FixedIpAlreadyInUse(EngineException):
...
class NoMoreFixedIps(EngineException):
...
class FloatingIpAssociated(EngineException):
    message = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(EngineException):
...
class NotAllowed(EngineException):
...
class ImageRotationNotAllowed(EngineException):
    message = _("Rotation is not allowed for snapshots")


class RotationRequiredForBackup(EngineException):
    message = _("Rotation param is required for backup image_type")


#TODO(bcwaldon): EOL this exception!
class Duplicate(EngineException):
    pass
...
class InvalidSharedStorage(EngineException):
    message = _("%(path)s is on shared storage: %(reason)s")


class MigrationError(EngineException):
    message = _("Migration error") + ": %(reason)s"


class MalformedRequestBody(EngineException):
...
class VSAEngineAccessParamNotFound(Invalid):
    message = _("Engine access parameters were not specified.")
...
class CannotResizeToSameSize(EngineException):
    message = _("When resizing, instances must change size!")


class ImageTooLarge(EngineException):
