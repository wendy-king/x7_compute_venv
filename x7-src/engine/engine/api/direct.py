...
class DelegatedAuthMiddleware(wsgi.Middleware):
    ...
    generate the appropriate engine.context.RequestContext for the rest of the
    ...
    def process_request(self, request):
        os_user = request.headers['X-X7-User']
        os_project = request.headers['X-X7-Project']
        context_ref = context.RequestContext(user_id=os_user,
                                             project_id=os_project)
        request.environ['x7.context'] = context_ref
...
class ServiceWrapper(object):
    ...
    Automatically used be engine.api.direct.Router to wrap registered instances.
    ...
    @webob.dec.wsgify(RequestClass=engine.api.x7.wsgi.Request)
    def __call__(self, req):
        arg_dict = req.environ['wsgiorg.routing_args'][1]
        action = arg_dict['action']
        del arg_dict['action']

        context = req.environ['x7.context']
        # allow middleware up the stack to override the params
        params = {}
        if 'x7.params' in req.environ:
            params = req.environ['x7.params']
        ...
        try:
            ...
            serializer = {
              'application/xml': engine.api.x7.wsgi.XMLDictSerializer(),
              'application/json': engine.api.x7.wsgi.JSONDictSerializer(),
            }[content_type]
