...
_MEDIA_TYPE_MAP = {
    'application/vnd.x7.compute+json': 'json',
    'application/json': 'json',
    'application/vnd.x7.compute+xml': 'xml',
    'application/xml': 'xml',
...
class Request(webob.Request):
    """Add some X7 API-specific logic to the base webob.Request."""

    def best_match_content_type(self):
        """Determine the requested response content-type."""
        if 'engine.best_content_type' not in self.environ:
            ...
            self.environ['engine.best_content_type'] = content_type or \
                'application/json'

        return self.environ['engine.best_content_type']
...
class ResponseHeadersSerializer(ActionDispatcher):
    ...
    def serialize(self, response, data, action):
        ...
        context = response.request.environ.get('engine.context')
        if context:
...
class ResponseSerializer(object):
    ...
    def serialize_body(self, request, response, data, content_type, action):
        ...
        if data is not None:
            ...
            lazy_serialize = request.environ.get('engine.lazy_serialize', False)
            if lazy_serialize:
                response.body = utils.dumps(data)
                request.environ['engine.serializer'] = serializer
                request.environ['engine.action'] = action
                if (hasattr(serializer, 'get_template') and
                    'engine.template' not in request.environ):

                    template = serializer.get_template(action)
                    request.environ['engine.template'] = template
            else:
...
class Resource(wsgi.Application):
    ...
    def __call__(self, request):
        ...
        if 'engine.context' in request.environ and project_id:
            request.environ['engine.context'].project_id = project_id
