...
LOG = logging.getLogger("engine.api.x7.v2.contrib.zones")
FLAGS = flags.FLAGS
...
class Controller(object):
    ...
    def info(self, req):
        ...
        items = engine.scheduler.api.get_zone_capabilities(context)
    ...
    def show(self, req, id):
        ...
        zone_id = int(id)
        context = req.environ['engine.context']
        zone = engine.scheduler.api.zone_get(context, zone_id)
        return dict(zone=_scrub_zone(zone))

    def delete(self, req, id):
        """Delete a child zone entry."""
        zone_id = int(id)
        engine.scheduler.api.zone_delete(req.environ['engine.context'], zone_id)
        return {}
    ...
    def create(self, req, body):
        ...
        context = req.environ['engine.context']
        zone = engine.scheduler.api.zone_create(context, body["zone"])
        return dict(zone=_scrub_zone(zone))
    ...
    def update(self, req, id, body):
        ...
        context = req.environ['engine.context']
        zone_id = int(id)
        zone = engine.scheduler.api.zone_update(context, zone_id, body["zone"])
        return dict(zone=_scrub_zone(zone))
    ...
    def select(self, req, body):
        ...
        ctx = req.environ['engine.context']
        specs = json.loads(body)
        build_plan = engine.scheduler.api.select(ctx, specs=specs)
...
class Zones(extensions.ExtensionDescriptor):
    ...
    name = "Zones"
    alias = "os-zones"
    namespace = "http://docs.x7.org/compute/ext/zones/api/v1.1"
