...
def find_config(config_path):
    ...
    :raises: `engine.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(FLAGS.state_path, "etc", "engine", config_path),
        os.path.join(FLAGS.state_path, "etc", config_path),
        os.path.join(FLAGS.state_path, config_path),
        "/etc/engine/%s" % config_path,
    ]
...
def enginedir():
    import engine
    return os.path.abspath(engine.__file__).split('engine/__init__.py')[0]


def default_flagfile(filename='engine.conf', args=None):
    if args is None:
        args = sys.argv
    for arg in args:
        ...
        if not os.path.exists(filename):
            ...
            if not os.path.exists(filename):
                filename = '/etc/engine/engine.conf'
...
def usage_from_instance(instance_ref, **kw):
    image_ref_url = "%s/images/%s" % (generate_tank_url(),
...
def monkey_patch():
    ...
    Example: 'engine.api.ec2.cloud:engine.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See engine.notifier.api.notify_decorator)
...
def generate_tank_url():
    """Generate the URL to tank."""
    # TODO(jk0): This will eventually need to take SSL into consideration
    # when supported in tank.
    return "http://%s:%d" % (FLAGS.tank_host, FLAGS.tank_port)
