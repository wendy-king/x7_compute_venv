...
class EngineLogger(logging.Logger):
    """EngineLogger manages request context and formatting.
...
class EngineFormatter(logging.Formatter):
    """A engine.context.RequestContext aware formatter configured through flags.
...
_formatter = EngineFormatter()


class EngineRootLogger(EngineLogger):
    def __init__(self, name, level=NOTSET):
        self.logpath = None
        self.filelog = None
        self.streamlog = None
        self.syslog = None
        EngineLogger.__init__(self, name, level)
...
class PublishErrorsHandler(logging.Handler):
    def emit(self, record):
        engine.notifier.api.notify('engine.error.publisher', 'error_notification',
            engine.notifier.api.ERROR, dict(error=record.msg))
...
def reset():
    ...
    for logger in EngineLogger.manager.loggerDict.itervalues():
        if isinstance(logger, EngineLogger):
