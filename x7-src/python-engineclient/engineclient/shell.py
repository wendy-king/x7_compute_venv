...
from engineclient import exceptions as exc
import engineclient.extension
from engineclient.keystone import shell as shell_keystone
from engineclient import utils
from engineclient.v1_1 import shell as shell_v1_1
...
class EngineClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(EngineClientArgumentParser, self).__init__(*args, **kwargs)
...
class X7ComputeShell(object):

    def get_base_parser(self):
        parser = EngineClientArgumentParser(
            prog='engine',
            description=__doc__.strip(),
            epilog='See "engine help COMMAND" '\
                   'for help on a specific command.',
            add_help=False,
            formatter_class=X7HelpFormatter,
        )
        ...
        parser.add_argument('--username',
            default=env('OS_USERNAME', 'ENGINE_USERNAME'),
            help='Defaults to env[OS_USERNAME].')

        parser.add_argument('--apikey',
            default=env('ENGINE_API_KEY'),
            help='Defaults to env[ENGINE_API_KEY].')

        parser.add_argument('--password',
            default=env('OS_PASSWORD', 'ENGINE_PASSWORD'),
            help='Defaults to env[OS_PASSWORD].')

        parser.add_argument('--projectid',
            default=env('OS_TENANT_NAME', 'ENGINE_PROJECT_ID'),
            help='Defaults to env[OS_TENANT_NAME].')

        parser.add_argument('--url',
            default=env('OS_AUTH_URL', 'ENGINE_URL'),
            help='Defaults to env[OS_AUTH_URL].')

        parser.add_argument('--region_name',
            default=env('ENGINE_REGION_NAME'),
            help='Defaults to env[ENGINE_REGION_NAME].')

        parser.add_argument('--endpoint_name',
            default=env('ENGINE_ENDPOINT_NAME'),
            help='Defaults to env[ENGINE_ENDPOINT_NAME] or "publicURL".')

        parser.add_argument('--version',
            default=env('ENGINE_VERSION'),
            help='Accepts 1.1, defaults to env[ENGINE_VERSION].')
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = engineclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self, version):
        for (module_loader, name, ispkg) in pkgutil.iter_modules():
            if name.endswith('python_engineclient_ext'):
    ...
    def do_bash_completion(self, args):
        ...
        engine.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
class X7HelpFormatter(argparse.HelpFormatter):
    def start_section(self, heading):
        # Title-case the headings
        heading = '%s%s' % (heading[0].upper(), heading[1:])
        super(X7HelpFormatter, self).start_section(heading)


def main():
    try:
        X7ComputeShell().main(sys.argv[1:])
