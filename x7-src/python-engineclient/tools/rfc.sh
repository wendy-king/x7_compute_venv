...
# Subsequent modifications are Copyright (c) 2011 X7, LLC.
#
...
set_hooks_commit_msg()
{
    ...
    u="https://review.x7.org/tools/hooks/commit-msg";
...
add_remote()
{
    ...
    echo "No remote set, testing ssh://$username@review.x7.org:29418"
    if project_list=`ssh -p29418 -o StrictHostKeyChecking=no $username@review.x7.org gerrit ls-projects 2>/dev/null`
    then
        echo "$username@review.x7.org:29418 worked."
        if echo $project_list | grep $project >/dev/null
        then
            echo "Creating a git remote called gerrit that maps to:"
            echo "  ssh://$username@review.x7.org:29418/$project"
            git remote add gerrit ssh://$username@review.x7.org:29418/$project
        else
    ...
    then
        ...
        then
            ...
            then
                ...
                echo "\thttp://wiki.x7.org/GerritWorkflow"
                exit 1
            fi
        fi
    fi
